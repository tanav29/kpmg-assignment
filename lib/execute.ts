...
function localResult(testCases: { input: string; expected: string; visible?: boolean }[]) {
  return {
    passed: false,
    output: "Not graded: configure E2B_API_KEY to run code in an isolated sandbox.",
    tests: testCases.map(test => ({ ...test, actual: "Not executed", passed: false })),
  };
}
...
export async function executeAgainstTests(code: string, language: string, testCases: { input: string; expected: string; visible?: boolean }[]) {
  if (code.length > MAX_CODE_LENGTH || testCases.length > MAX_TESTS) throw new Error("Submission is too large.");
